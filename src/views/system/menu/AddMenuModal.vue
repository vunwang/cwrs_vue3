...
import {getMenuDetail, addDirectory, addMenu, addButton, editDirectory, editMenu, editButton} from '@/apis/system'
...
const [form, resetForm] = useResetReactive<MenuForm>({
  ...
  hidden: false, // 设置 true 的时候该路由不会在侧边栏出现
  parentId: '',
  redirect: '', // 重定向
  affix: false, // 固定在标签页
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      if (form.type === '1') {
        ...
        res = await editDirectory(form)
      } else if (form.type === '2') {
        res = await editMenu(form)
      } else if (form.type === '3') {
        res = await editButton(form)
      }
    } else {
      //添加
      if (form.type === '1') {
        form.component = 'Layout'
        res = await addDirectory(form)
      } else if (form.type === '2') {
        res = await addMenu(form)
      } else if (form.type === '3') {
        res = await addButton(form)
      }
    }
    if (res) {
