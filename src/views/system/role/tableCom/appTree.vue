  ...
  <div>
    ...
    <div class="menu-permission-header">
      <div class="dis_flex_center">
        ...
        <div class="fontStyle14">APP菜单权限</div>
      </div>
...
const treeAppReady = ref(false);
const checkedMenuIds = ref<string[]>([]);
const treeAppRef = ref()
const deptTreeAppRef = ref();
...
// 同步props到内部数据
watch(
    () => [props.menuAppData, props.roleMenusApp],
...
const handleCheckedTreeNodeAll = (val) => {
  ...
  treeAppRef.value.setCheckedKeys(allKeys)
}

/** 树权限（展开/折叠） */
const onExpandAllChange = (val: boolean) => {
  const tree = treeAppRef.value
  if (!tree) return
  // 递归展开所有节点
...
const handleTreeData = () => {
  let allNodes = []
  if (!isCheckStr.value) {
    const checkedKeys = treeAppRef.value.getCheckedKeys();
    ...
    // 重新设置选中状态（包含子节点 + 所有祖先）
    treeAppRef.value.setCheckedKeys(Array.from(newCheckedSet));
    allNodes = treeAppRef.value.getCheckedNodes();
  } else {
    const checkedNodes = treeAppRef.value.getCheckedNodes();
    const halfCheckedNodes = treeAppRef.value.getHalfCheckedNodes();
    // 合并全选 + 半选（此时半选就是父节点）
...
const handleCheckChange = () => {
  const selectedDeptIds = deptTreeAppRef.value.getCheckedKeys();
...
const handleOpen = (data: any) => {
  ...
  getDeptList()
      .then((res) => {
        ...
        // 等待树渲染完成后设置选中状态
        nextTick(() => {
          deptTreeAppRef.value?.setCheckedKeys(selectedDeptIds);
        });
      })
      .catch((error) => {
...
const reset = () => {
  // 重置表格状态
  treeAppRef.value?.setCheckedKeys([]);
  isExpandAll.value = false;
  dataPurviewAll.value = "";
  // 重置弹窗状态
  dialogTableVisible.value = false;
  // 重置内部菜单数据（恢复为props原始值）
  innerMenuData.value = deepCloneWithMenuIdString(props.menuAppData);
};
const getRoleAppMenus = () => {
  return menuTreeRes.value
};


//  暴露方法给父组件
defineExpose({reset, getRoleAppMenus});
</script>
